          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_460px] lg:items-center">
            <div>
              ...
              <div className="mt-8 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/projects" className="inline-flex justify-center rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-brand-panel px-7 py-3 text-sm font-semibold text-brand-ink dark:text-slate-100 transition hover:border-slate-500">Lihat Project</Link>
              </div>
...
function BuilderWorkspaceCard() {
  return (
    <article className="rounded-[24px] border border-slate-200 dark:border-brand-line bg-white dark:bg-brand-panel p-5 shadow-[0_24px_80px_rgba(15,23,42,0.08)]">
      ...
      <div className="mt-5 rounded-2xl border border-brand-border bg-brand-soft p-5">
        ...
        <div className="mt-6 grid gap-3">
          {features.map((feature) => <div key={feature} className="flex items-center gap-3 rounded-xl bg-white dark:bg-brand-panel/80 p-3 text-sm text-slate-700 dark:text-slate-300"><span className="grid h-5 w-5 place-items-center rounded-full bg-emerald-500 text-[11px] text-white">✓</span>{feature}</div>)}
        </div>
...
function MiniStat({ label, value }: Readonly<{ label: string; value: string }>) {
  return <div className="rounded-xl border border-slate-200 dark:border-brand-line bg-slate-50 dark:bg-slate-900 p-3"><p className="font-mono text-[10px] uppercase tracking-wide text-slate-500 dark:text-slate-400">{label}</p><p className="mt-1 text-xs font-semibold text-brand-ink dark:text-slate-100">{value}</p></div>;
}
...
function HomeToolCard({ tool }: Readonly<{ tool: (typeof tools)[number] }>) {
  ...
  return (
    <article className="group rounded-[20px] border border-slate-200 dark:border-brand-line bg-white dark:bg-brand-panel p-6 transition hover:-translate-y-1 hover:border-slate-300 dark:border-slate-600">
      <div className="flex items-start justify-between gap-3">
        <div className={`grid h-11 w-11 place-items-center rounded-xl text-xs font-semibold ${available ? "bg-brand-soft text-brand-primary" : "bg-slate-100 text-slate-500 dark:text-slate-400"}`}>{tool.icon}</div>
